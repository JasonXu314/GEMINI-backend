...
export class CreateModelDto {
	...
	@HasMimeType(['application/octet-stream'], {
		message: 'struct file is not of correct format, must be application/octet-stream'
	})
	structure: MemoryStoredFile;

	@IsFile({ message: 'epiData property must be a epi file' })
	@HasMimeType(['application/octet-stream'], {
		message: 'epi file is not of correct format, must be application/octet-stream'
	})
	epiData: MemoryStoredFile;

	@IsFile({ message: 'refGenes property must be a gref file' })
	@HasMimeType(['application/octet-stream'], {
		message: 'gref file is not of correct format, must be application/octet-stream'
	})
